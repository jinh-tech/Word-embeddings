...
class BPTF(BaseEstimator, TransformerMixin):
    def __init__(self, n_modes, n_components,  max_iter, tol,smoothness, verbose, alpha, n_trunc, alpha_prime=0.1, beta_prime=1.0):
    ...
    def _update_B(self,data):
        ...
        self.E_B = (self.E_N + 1.0) / (data.vals + 1.0)
        self.G_B = sp.psi(self.E_N + 1.0) - np.log(data.vals + 1.0)     # RHS should be equal to log(self.G_B)
...
def main():
    ...
    p.add_argument('-t', '--tol', type=float, default=0.0)
    p.add_argument('-s', '--smoothness', type=int, default=2)
    ...
    serialize_bptf(bptf, args.out, desc=args.info, algo_name="glove-hcpf")
