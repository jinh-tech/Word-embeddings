...
class BPTF(BaseEstimator, TransformerMixin):
    ...
    def _update_N(self,data):
        ...
        #temp = np.log(2.0) + np.log(self._reconstruct_nz(data.subs,self.G_DK_M))
    ...
    def _update_B(self,data):
        ...
        self.E_B = (self.E_N + 2.0) / (data.vals + 1.0)
        self.G_B = sp.psi(self.E_N + 2.0) - np.log(data.vals + 1.0)     # RHS should be equal to log(self.G_B)
...
def main():
    ...
    p.add_argument('-s', '--smoothness', type=int, default=100.0)
    ...
    print np.sum(data.vals)/data.vals.size
    del ind_tup
    del val_tup
    del data_dict
    ...
    serialize_bptf(bptf, args.out, desc=args.info, algo_name="svd-hcpf")
